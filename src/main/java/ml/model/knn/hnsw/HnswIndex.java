...
import com.rj.ml.model.knn.*;
import com.rj.ml.model.knn.distance.DistanceFunction;
import com.rj.ml.model.knn.listener.ProgressListener;
import com.rj.ml.model.knn.serializer.JavaObjectSerializer;
import com.rj.ml.model.knn.serializer.ObjectSerializer;
import com.rj.ml.model.knn.util.*;
...
public class HnswIndex<TId, TVector, TItem extends Item<TId, TVector>, TDistance>
    ...
    public boolean add(TItem item) {
        if (item.dimensions() != dimensions) {
            System.out.println("Item: " + item.id() + " does not have dimensionality of : " + dimensions);
            return false;
        }

        int randomLevel = assignLevel(item.id(), this.levelLambda);

        IntArrayList[] connections;
        try {
            connections = new IntArrayList[randomLevel + 1];
        } catch (NegativeArraySizeException e) {
            System.out.println("Item: " + item.id() + " random level < 0");
            return false;
        }
